...
def load_orig_images(images_root, split):
    root = Path(images_root).expanduser().resolve() / split
    index = root / 'index.txt'
    if split == 'train':
      index = root / 'random_index.txt'
    if not index.exists():
        raise(RuntimeError("index.txt does not exist in {}".format(root)))
    
    image_files = []
    with index.open() as f:
        for fname in f:
            fname = root / fname.strip()
...
class TranslationTask(FairseqTask):
    ...
    def build_dataset_for_inference(self, src_tokens, src_lengths, split):
