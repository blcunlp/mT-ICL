...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, awl, dummy_batch=None, oom_batch=None):
        self.args = args
        self.task = task
    ...
    def init_meters(self, args):
        ...
        self.meters["train_triplet_loss"] = AverageMeter()
        self.meters["valid_loss"] = AverageMeter()
        self.meters["valid_nll_loss"] = AverageMeter()
        self.meters["valid_triplet_loss"] = AverageMeter()
        ...
        if self.awl is not None:
            params = list(
                filter(
                    ...
                    chain(self.model.parameters(), self.criterion.parameters(), self.awl.parameters()),
                )
            )
        else:
            params = list(
    ...
    def train_step(self, samples, epoch, dummy_batch=False, raise_oom=False):
        ...
        try:
            ...
            if "triplet_loss" in logging_output:
                self.meters["train_triplet_loss"].update(
                    logging_output.get("triplet_loss", 0), ntokens
                )
    ...
    def valid_step(self, sample, epoch, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            try:
                _loss, sample_size, logging_output = self.task.valid_step(
                    sample, self.model, self.criterion, self.awl
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    self._log_oom(e)
        ...
        if "triplet_loss" in logging_output:
            self.meters["valid_triplet_loss"].update(
                logging_output.get("triplet_loss", 0), ntokens
            )
    ...
    def epoch_step(self, epoch, val_loss=None):
        """Adjust the learning rate at end of each epoch."""
