...
class simple_progress_bar(progress_bar):
    ...
    def __iter__(self):
        ...
        for i, obj in enumerate(self.iterable, start=self.offset):
            ...
            if self.stats is not None and i > 0 and \
                ...
                print('{} {}:  {:5d} / {:d} {}'.format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'), self.prefix, i, size, postfix), flush=True)
    ...
    def print(self, stats, tag='', step=None):
        ...
        print('{} {} | {}'.format(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'),self.prefix, postfix), flush=True)
