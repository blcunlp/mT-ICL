...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def forward(self, x, encoder_padding_mask, attn_mask=None, img_emb=None, txt_len=None):
        """
        Args:
            ...
            New_added: txt_len: the length of text tokens 

        Returns:
            encoded output of shape `(seq_len, batch, embed_dim)`
        """
        # x (seq_len+pixel, batch, embed_dim)
        ...
        #use [:txt_len] of previous encoder output as K, V
        x, attn  = self.self_attn(query=x, key=x[:txt_len,:,:], value=x[:txt_len,:,:], key_padding_mask=encoder_padding_mask)
